...
TerrainShaderClass::TerrainShaderClass(TerrainShaderClass& other)
{
}
...
{
	bool result;
	result = InitializeShader(device, hwnd, L"../Engine/heightmap.vs", L"../Engine/heightmap.ps");
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "HeightmapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "HeightmapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
...
void TerrainShaderClass::RenderShader(ID3D11DeviceContext* devCon, int indexCount)
{
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* devCon, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView** texture)
{
	HRESULT result;
	...
	devCon->VSSetShaderResources(0, 1, texture);

	// Now set the constant buffer in the vertex shader with the updated values.
    devCon->VSSetConstantBuffers(bufferNumber, 1, &m_matrixBuffer);
	
	// Set shader texture resource in the pixel shader.
	devCon->PSSetShaderResources(0, 3, texture + 1);
