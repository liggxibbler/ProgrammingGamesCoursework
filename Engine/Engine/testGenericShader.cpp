...
bool TestGenericShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	result = InitializeShader(device, hwnd, L"../Engine/bitmap.vs", L"../Engine/bitmap.ps");
	if(!result)
	{
		return false;
	}
...
bool TestGenericShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
bool TestGenericShader::SetShaderParameters(ID3D11DeviceContext* devCon)
{
	HRESULT result;
	...
	D3DXMATRIX worldMatrix = *(PopMatrix());
	D3DXMATRIX viewMatrix = *(PopMatrix());
	D3DXMATRIX projectionMatrix = *(PopMatrix());
	...
	ID3D11ShaderResourceView* texture = PopResourceView();
