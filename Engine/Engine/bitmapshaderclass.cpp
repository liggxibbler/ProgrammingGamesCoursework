...
bool BitmapShaderClass::Render(ID3D11DeviceContext* devCon, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView** texture)
{
	bool result;
	result = SetShaderParameters(devCon, worldMatrix, projectionMatrix, texture);
	if(!result)
	{
		return false;
	}
...
bool BitmapShaderClass::SetShaderParameters(ID3D11DeviceContext* devCon, D3DXMATRIX worldMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView** texture)
{
	HRESULT result;
